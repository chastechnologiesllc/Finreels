...
The Web document now paints the complete dependency-free branded splash before Flutter JavaScript finishes loading. Web no longer builds the Flutter splash widget: Flutter renders a transparent interim frame, then dispatches a readiness event after the first usable shell/onboarding frame so the HTML splash remains visible through handoff. The native Flutter splash uses the same “Opening your learning space…” status wording. These changes reduce artificial waiting and make slow cold starts feel intentional, but they cannot eliminate browser network transfer, Flutter engine initialization, device scheduling, or a user’s connection latency.
