# Rumuo General Library Additions — 25 August 2026
...
The generated asset `assets/books/rumuo_book_cover_fallback.png` is a branded **Rumuo Open Library** cover. `BookCoverImage` now uses it for missing, empty, or exhausted cover candidates, and retains the small icon-only fallback only if the bundled asset itself cannot be decoded. Exact-edition candidates remain identifier-driven: ISBN URLs are allowed when the ISBN is verified by the source record, while the three records without an independently verified ISBN cover intentionally use the branded fallback rather than an unrelated title search result.
