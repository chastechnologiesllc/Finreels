...
| iOS delivery | iOS deployment target is 13.0; orientation, Workmanager registration, plugin registration, and ATS policy are present. New CI compiles iOS without signing. | Pass with device/archive limitation |
...
The new iOS workflow compiles with `--no-codesign`; it does not replace Apple signing, App Store provisioning, simulator testing, or testing on physical browsers/devices. Browser compatibility is covered by the Flutter Web build and shared code review, but no automated matrix of every browser engine is claimed.
